...
{
    checkAlertRemAds=YES;
    checkAlertZulu=YES;
//    NSLog(@"tarnsaction Receipt -- %@",transaction.transactionReceipt);
...
{
    //NSLog(@"transaction.transactionReceipt length == %d  \n\n transaction.transactionReceipt == %@",[transaction.transactionReceipt length], transaction.transactionReceipt);
...

{
    ...
    int failCount = 0;
    NSLog(@"Transaction.payment.productIdentifier == %@ /n/n purchaseInfoDict product-id == %@",transaction.payment.productIdentifier,[purchaseInfoDict objectForKey:@"product-id"]);
    NSLog(@"transaction.payment.quantity == %ld /n /n transaction.transactionIdentifier == %@",(long)transaction.payment.quantity,transaction.transactionIdentifier);
...
//        completionHandler(TRUE);
        NSDictionary *dict = [deserializedDictionary objectForKey:@"receipt"];
        ...
        if ([strProductId isEqualToString:@"com.globussoft.multiarrows"]) {
            
            [self saveArrowPurchaseDetails:dict withType:@"MultipleArrow"];
            int multiArrowCount = 5;
            NSString *multiArrowString = [NSString stringWithFormat:@"%i",multiArrowCount];
            [keyChainMultiArrow setObject:multiArrowString forKey:(__bridge id)kSecValueData];
        }
        else if ([strProductId isEqualToString:@"com.globussoft.powerboosterarrows"]) {
            
            [self saveArrowPurchaseDetails:dict withType:@"BoosterArrow"];
            int powerBoosterArrowCount = 3;
            
            NSString *multiArrowString = [NSString stringWithFormat:@"%i",powerBoosterArrowCount];

             [keyChainMultiArrow setObject:multiArrowString forKey:(__bridge id)kSecAttrAccount];
        }
        
        else if ([strProductId isEqualToString:@"com.globussoft.newLife"]) {
            
            [self saveLifePurchaseDetails:dict];
            int lifeCount = 5;
            
            NSString *lifeString = [NSString stringWithFormat:@"%i",lifeCount];
            [keyChainMultiArrow setObject:lifeString forKey:(__bridge id)kSecAttrLabel];
        }
        else if ([strProductId isEqualToString:@"com.globussoft.arrows"]){
            [self saveArrowPurchaseDetails:dict withType:@"Arrow"];
            if (self.delegate != nil && [self.delegate respondsToSelector:@selector(arrowPurchaseValidation:)]) {
                [self.delegate arrowPurchaseValidation:YES];
            }
        }
    }
...
//        completionHandler(FALSE);
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(arrowPurchaseValidation:)]) {
            [self.delegate arrowPurchaseValidation:NO];
        }
    }
}
...
-(void)saveArrowPurchaseDetails:(NSDictionary *)dict withType:(NSString *)type{
    NSString * transaction_id  =[dict objectForKey:@"transaction_id"];
  //  NSString * purchase_date =[dict objectForKey:@"purchase_date"];
    NSString *uduid = [dict objectForKey:@"unique_identifier"];
